...
def test_classify_user_prompt():
    ...
    assert "SET_STANDBY_BUCKET" in battery_result["actionable_focus"]
    ...
    assert "RESTRICT_BACKGROUND_DATA" in data_result["actionable_focus"]
    ...
    assert empty_result["is_relevant"] == True
    
    # Test None prompt
    none_result = classify_user_prompt(None)
    assert none_result["is_relevant"] == True
